...
public:
	...
	void sizeSort(int &n, customerStatus* max)
	{
...
void imp_res::RED(string name, int energy)
{
	...
	if(numTable == 0)
	{
		...
		numTable++;
		tailStatus = newCusStatus;
		headStatus = newCusStatus;
		return;
	}
	...
	else
	{
		...
		bool check = energy < customerX->energy;
		switch(check)
		{
			case true:
			{
...
void imp_res::DOMAIN_EXPANSION()
{
	...
	while (numQueue > 0 && numTable < MAXSIZE)
	{
		...
		else
		{
			...
			// bool check = newCus->energy < customerX->energy;
			// switch(check)
			// {
			// 	case true:
			// 	{
