...
// 			unsigned long long pascal[n+1][n+1];
// 			for (int i = 0; i <= n; i++){
// 				for (int j = 0; j <= i; j++){
// 					if (j == 0 || j == i){
// 						pascal[i][j] = 1;
...
// 			return permutationFormula(countLeft + countRight, countLeft) * DFS(node->left) * DFS(node->right);
// 		}
// 		// void postorder(Node* node, vector<int>& nums)
// 		// {
// 		// 	if (node == nullptr) return;
// 		// 	postorder(node->left, nums);
// 		// 	postorder(node->right, nums);
// 		// 	nums.push_back(node->result);
// 		// }
