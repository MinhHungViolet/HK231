...
public:
	...
	vector<pair<char, int>> readName(string& name)
	{
		...
		for (char c : name){
            bool found = false;
            for (std::size_t i = 0; i < prefq.size(); i++){
                if (prefq[i].first == c){
                    prefq[i].second++;
                    found = true;
                    break;
		...
		for (std::size_t i = 0; i < name.length(); i++){
            int shift = 0;
            for (std::size_t j = 0; j < prefq.size(); j++){
		...
		for(std::size_t i = 1; i < prefq.size(); i++) {
			bool found = false;
			for(std::size_t j = 0; j < fq.size(); j++) {
	...
	Node* buildHuff(vector<pair<char, int>> fq)
	{
		...
		while(forestNode.size() > 1)
		{
			...
			int index = -1;
			for(std::size_t i = 0; i < forestNode.size(); i++) {
...
private:
	...
	list<Node* > LRU; 
private:
	void ReHeap_down(std::size_t index){
		...
		if (rightChild < std::size_t (area.size()) && 
...
public:
	...
	void print_preorder(std::size_t index, int number)
	{	
...
private:
	...
	public:
		...
		unsigned long long tohop(std::size_t x, std::size_t n){
			if(x == 0 || x == n) return 1;
